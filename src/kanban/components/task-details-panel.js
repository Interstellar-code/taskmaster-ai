	...
	render() {
		...
		if (this.task.dependencies && this.task.dependencies.length > 0) {
			const depsText =
				...
				chalk.yellow(this.task.dependencies.join(', '));
			lines.push(
				createVerticalLine(
					truncateText(depsText, this.contentWidth),
					this.width
				)
			);
		}
		...
		if (this.task.prdSource) {
			const prdText =
				chalk.gray('PRD Source: ') +
				chalk.cyan(this.task.prdSource.fileName || 'Unknown');
			lines.push(
				createVerticalLine(truncateText(prdText, this.contentWidth), this.width)
			);
		}
		...
		if (this.task.subtasks && this.task.subtasks.length > 0) {
			const subtasksText =
				chalk.gray('Subtasks: ') +
				chalk.magenta(`${this.task.subtasks.length} items`);
		...
		if (this.task.description) {
			...
			lines.push(
				createVerticalLine(chalk.white.bold('Description:'), this.width)
			);
		...
		if (this.task.details) {
			...
			if (detailLines.length > 3) {
				lines.push(
					createVerticalLine(chalk.dim('... (truncated)'), this.width)
				);
			}
		}
