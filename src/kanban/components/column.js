...
export class KanbanColumn {
	...
	getScrollInfo() {
		...
		const endTask = Math.min(
			this.scrollOffset + this.maxVisibleTasks,
			this.tasks.length
		);
		...
		return {
			...
			canScrollDown:
				this.scrollOffset < this.tasks.length - this.maxVisibleTasks
		};
	}
	...
	render(maxTasksToShow = 10) {
		...
		if (tasksToShow.length === 0) {
			...
			for (
				let i = 0;
				i < tasksToShow.length && currentLine < contentHeight - 2;
				i++
			) {
	...
	renderTaskCard(task, isSelected = false) {
		...
		if (isSelected) {
			...
			if (this.width < 30) {
				...
				const metadataLine = createMetadataLine(
					task,
					cardWidth - 2,
					this.width < 40
				);
				...
				cardLines.push(
					createVerticalLine(chalk.bgBlue.white(titleLine), cardWidth)
				);
				if (metadataLine) {
					cardLines.push(
						createVerticalLine(chalk.bgBlue.white(metadataLine), cardWidth)
					);
				}
			}
