...
export class HelpOverlay {
    ...
    render() {
        ...
        for (const section of currentPageData.sections) {
            ...
            // Section title
            lines.push(createVerticalLine(chalk.white.bold(section.title + ':'), helpWidth));
            contentLines++;
        ...
        lines.push(createHorizontalLine(helpWidth, BOX_CHARS.leftTee, BOX_CHARS.rightTee));
        const navText = 'PgUp/PgDn: Navigate pages | ESC/H: Close help';
        lines.push(createVerticalLine(chalk.cyan(centerText(navText, helpWidth - 2)), helpWidth));
        
        // Bottom border
        lines.push(createHorizontalLine(helpWidth, BOX_CHARS.bottomLeft, BOX_CHARS.bottomRight));
    ...
    getContextHelp(mode = 'normal') {
        const contextHelp = {
            normal: 'H: Help | ←→: Columns | ↑↓: Tasks | 1-3: Move | V: View | Q: Quit',
