...
export class TaskCard {
     ...
     */
    createTopBorder() {
        return createHorizontalLine(this.width, BOX_CHARS.topLeft, BOX_CHARS.topRight);
    }
     ...
     */
    createBottomBorder() {
        return createHorizontalLine(this.width, BOX_CHARS.bottomLeft, BOX_CHARS.bottomRight);
    }
    ...
    createMetadataLine() {
        ...
        metadata.push(`${priorityDisplay.symbol} ${priorityDisplay.color(priorityDisplay.text)}`);
    ...
    renderCompact() {
        ...
        // Calculate available space for title
        const reservedSpace = stripAnsi(idText).length + stripAnsi(priorityText).length + 2; // +2 for spaces
    ...
    renderDetailed() {
        ...
        // PRD source line
        if (this.task.prdSource) {
            const prdText = chalk.blue('📄 ') + chalk.gray(this.task.prdSource.fileName || 'PRD Source');
            lines.push(createVerticalLine(prdText, this.width));
        }
        
        // Subtasks line
        if (this.task.subtasks && this.task.subtasks.length > 0) {
            const subtaskText = chalk.magenta(`📝 ${this.task.subtasks.length} subtasks`);
 ...
 */
export function renderTaskCards(tasks, columnWidth, selectedIndex = -1, maxCards = 10) {
    const cards = [];
