...
export class TaskDetailsPopup {
	...
	generateContent() {
		...
		if (this.task.dependencies && this.task.dependencies.length > 0) {
			metadataItems.push(
				`   🔗 Dependencies: ${chalk.yellow(this.task.dependencies.join(', '))}`
			);
		}

		if (this.task.prdSource) {
			metadataItems.push(
				`   📄 PRD Source: ${chalk.cyan(this.task.prdSource.fileName || 'Unknown')}`
			);
			if (this.task.prdSource.filePath) {
				metadataItems.push(
					`   📁 Path: ${chalk.gray(this.task.prdSource.filePath)}`
				);
			}
		}
		...
		if (this.task.subtasks && this.task.subtasks.length > 0) {
			metadataItems.push(
				`   📝 Subtasks: ${chalk.magenta(`${this.task.subtasks.length} subtasks`)}`
			);
		}
		...
		sections.forEach((section) => {
			if (section.content) {
				...
				lines.forEach((line) =>
					this.contentLines.push(`   ${section.color(line)}`)
				);
				this.contentLines.push('');
		...
		if (subtasks.length > 0) {
			...
			subtasks.forEach((subtask) => {
				...
				wrappedLines.forEach((line) =>
					this.contentLines.push(chalk.gray(line))
				);
			});
