...
import { getTerminalSize, truncateText, stripAnsi } from '../utils/terminal-utils.js';
...
export class StatusBar {
    ...
    getModeInfo(boardState) {
        switch (this.mode) {
            case 'filter':
                const activeFilters = Object.keys(boardState.activeFilters || {}).length;
    ...
    createProgressBar(width = 20) {
        const percentage = Math.round((this.progressValue / this.progressMax) * 100);
        const filledWidth = Math.round((this.progressValue / this.progressMax) * width);
        const emptyWidth = width - filledWidth;
        
        const filled = '█'.repeat(filledWidth);
        const empty = '░'.repeat(emptyWidth);
        
        return chalk.green(filled) + chalk.gray(empty) + chalk.white(` ${percentage}%`);
    }
    ...
    renderMinimal(boardState, width) {
        ...
        return chalk.bgBlue.white(' ' + truncateText(content, width - 2).padEnd(width - 2) + ' ');
    }
}
