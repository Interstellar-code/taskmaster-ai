...
export class EditModal {
	...
	render() {
		...
		lines.push(
			createVerticalLine(
				chalk.cyan(this.truncateText(taskInfo, this.contentWidth)),
				this.width
			)
		);
		...
		if (this.editType === 'priority') {
			lines.push(
				createVerticalLine(
					chalk.dim('↑↓ to change, Enter to save, Esc to cancel'),
					this.width
				)
			);
		} else {
			lines.push(
				createVerticalLine(
					chalk.dim('Type to edit, Enter to save, Esc to cancel'),
					this.width
				)
			);
		}
	...
	renderPrioritySelector() {
		...
		priorities.forEach((priority, index) => {
			...
			if (isSelected) {
				lines.push(
					createVerticalLine(chalk.bgBlue.white(`> ${text}`), this.width)
				);
			} else {
