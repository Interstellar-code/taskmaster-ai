...
export class TaskOperationsHandler {
    ...
    async viewTaskDetails(task) {
        ...
        try {
            ...
            console.log(chalk.blue(`│ Title: ${chalk.white(task.title || 'Untitled Task')}`));
            console.log(chalk.blue(`│ Status: ${this.getStatusDisplay(task.status)}`));
            console.log(chalk.blue(`│ Priority: ${this.getPriorityDisplay(task.priority)}`));

            if (task.dependencies && task.dependencies.length > 0) {
                console.log(chalk.blue(`│ Dependencies: ${chalk.yellow(task.dependencies.join(', '))}`));
            }

            if (task.prdSource) {
                console.log(chalk.blue(`│ PRD Source: ${chalk.cyan(task.prdSource.fileName || 'Unknown')}`));
            }

            if (task.subtasks && task.subtasks.length > 0) {
                console.log(chalk.blue(`│ Subtasks: ${chalk.magenta(task.subtasks.length)}`));
            }
    ...
    async editTaskTitle(task) {
        ...
        try {
            ...
            return { 
                success: false, 
                reason: 'Title editing requires input capture (full implementation pending)',
                requiresInput: true 
            };
