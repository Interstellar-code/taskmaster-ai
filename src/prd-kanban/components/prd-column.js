...
export class PRDKanbanColumn {
	...
	getSelectedPRD() {
		if (
			this.selectedPRDIndex >= 0 &&
			this.selectedPRDIndex < this.prds.length
		) {
	...
	render(maxPRDsToShow = 10) {
		...
		if (useDouble) {
			...
			lines.push(
				createDoubleHorizontalLine(this.width, true)
					.replace(BOX_CHARS.doubleTopLeft, BOX_CHARS.doubleLeftTee)
					.replace(BOX_CHARS.doubleTopRight, BOX_CHARS.doubleRightTee)
			);
		} else if (useRounded) {
		...
		if (prdsToShow.length === 0) {
			...
			if (scrollInfo && scrollInfo.canScrollUp) {
				...
				if (useDouble) {
					lines.push(
						createDoubleVerticalLine(
							centerText(scrollUpIndicator, this.width - 4),
							this.width
						)
					);
				} else {
			...
			for (
				let i = 0;
				i < prdsToShow.length && currentLine < contentHeight - 2;
				i++
			) {
			...
			if (scrollInfo && scrollInfo.canScrollDown) {
				...
				if (useDouble) {
					lines.push(
						createDoubleVerticalLine(
							centerText(scrollDownIndicator, this.width - 4),
							this.width
						)
					);
				} else {
	...
	renderPRDCard(prd, isSelected = false) {
		...
		if (isSelected) {
			...
			if (this.width < 30) {
				...
				cardLines.push(
					createVerticalLine(chalk.bgBlue.white(compactLine), cardWidth)
				);
			} else {
				...
				lines.forEach((line) => {
					cardLines.push(
						createVerticalLine(chalk.bgBlue.white(line), cardWidth)
					);
				});
			}
	...
	createEnhancedPRDContent(prd, maxWidth) {
		...
		const completion = prd.taskStats
			? prd.taskStats.completionPercentage || 0
			: prd.completion || 0;
		const taskCount = prd.taskStats
			? prd.taskStats.totalTasks || 0
			: prd.linkedTaskIds
	...
	createPRDMetadataLine(prd, maxWidth, isCompact) {
		...
		if (isCompact) {
			return chalk.dim(
				`${statusIcon}${priorityIcon}${complexityIcon} ${taskCountText} ${completionText}`
			);
		} else {
			return chalk.dim(
				`${statusIcon} ${priorityIcon} ${complexityIcon} ${taskCountText} ${completionText}`
