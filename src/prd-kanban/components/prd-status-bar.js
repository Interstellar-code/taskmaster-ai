...
export class PRDStatusBar {
	...
	buildStatusBarContent(boardState, width) {
		...
		if (boardState.selectedPRD) {
			...
			const truncatedInfo =
				prdInfo.length > 40 ? prdInfo.substring(0, 37) + '...' : prdInfo;
		...
		if (boardState.statusCounts) {
			...
			parts.push(
				`P:${counts.pending || 0} | I:${counts['in-progress'] || 0} | D:${counts.done || 0}`
			);
		}

		// Navigation hints
		parts.push(
			'←→: Columns | ↑↓: PRDs | V: Details | T: Tasks | H: Help | Q: Quit'
		);
	...
	updateNavigationInfo(navigationHandler) {
		if (!navigationHandler) {
			this.setContent(
				'PRD Kanban Board - Use arrow keys to navigate, Q to quit'
			);
			return;
		}
		...
		if (selectedPRD) {
			...
			this.setContent(
				`Column: ${positionInfo.column} | No PRDs | ` + `←→:Columns Q:Quit`
			);
		}
	}
	...
	showColumnInfo(columnName, prdCount, selectedIndex) {
		const position =
			selectedIndex >= 0 ? ` (${selectedIndex + 1}/${prdCount})` : '';
