...
export class PRDBoardLayout {
	...
	generateBoardLines() {
		...
		const contentLines = this.generateColumnContent(
			columnWidth,
			padding,
			boardHeight - 1
		);
	...
	generateColumnHeaders(columnWidth, padding) {
		const headers = this.statusOrder.map((status, index) => {
			...
			const headerPadding = Math.max(
				0,
				Math.floor((columnWidth - header.length) / 2)
			);
			const paddedHeader =
				' '.repeat(headerPadding) +
				header +
				' '.repeat(columnWidth - headerPadding - header.length);
