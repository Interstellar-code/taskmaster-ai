...
export class PRDDetailsPopup {
	...
	async generateContent() {
		...
		const titleLines = this.wrapText(
			this.prd.title || 'Untitled PRD',
			contentWidth - 2
		);
		titleLines.forEach((line) =>
			this.contentLines.push(`   ${chalk.cyan(line)}`)
		);
		this.contentLines.push('');
		...
		if (this.prd.complexity) {
			metadataItems.push(
				`   🔧 Complexity: ${this.getComplexityDisplay(this.prd.complexity)}`
			);
		}

		if (this.prd.completion !== undefined) {
			metadataItems.push(
				`   📈 Completion: ${chalk.cyan(this.prd.completion + '%')}`
			);
		}

		if (this.prd.linkedTasks && this.prd.linkedTasks.length > 0) {
			metadataItems.push(
				`   🔗 Linked Tasks: ${chalk.yellow(this.prd.linkedTasks.length + ' tasks')}`
			);
		}
		...
		if (this.prd.description) {
			...
			descLines.forEach((line) =>
				this.contentLines.push(`   ${chalk.white(line)}`)
			);
			this.contentLines.push('');
	...
	async addLinkedTasksSection(contentWidth) {
		...
		try {
			...
			const linkedTasks = allTasks.filter((task) => {
				...
				if (
					this.prd.linkedTaskIds &&
					this.prd.linkedTaskIds.includes(task.id)
				) {
					return true;
				}
			...
			if (linkedTasks.length > 0) {
				...
				this.contentLines.push(
					chalk.dim('   No tasks found linked to this PRD')
				);
				this.contentLines.push('');
				this.contentLines.push(''); // Extra spacing
			}
			...
			this.contentLines.push(
				chalk.red('   Error loading linked tasks: ' + error.message)
			);
			this.contentLines.push('');
			this.contentLines.push(''); // Extra spacing
		}
	...
	getComplexityDisplay(complexity) {
		...
		return (
			complexityMap[complexity] || chalk.gray(`⚪ ${complexity || 'None'}`)
		);
	}
	...
	render() {
		...
		// Title bar
		const titleBar = centerText(
			chalk.blue.bold(` PRD Details `),
			this.popupWidth - 2
		);
		...
		// Separator
		lines.push(
			createRoundedHorizontalLine(this.popupWidth, true, false)
				.replace(/╭/g, '├')
				.replace(/╮/g, '┤')
		);
		...
		const centeredBottomText = centerText(
			chalk.dim(bottomText),
			this.popupWidth - 2
		);
