...
export class PRDKeyboardHandler {
	...
	setupDefaultKeyMappings() {
		...
		this.keyHandlers.set('a', () => {
			try {
				...
				console.error(
					chalk.red('Error showing archive confirmation:'),
					error.message
				);
			}
		});
	...
	start() {
		...
		console.log(
			chalk.dim(
				'Keyboard handler started. Use arrow keys to navigate, Q to quit.'
			)
		);
	}
	...
	handleKeyPress(key) {
		...
		try {
			...
			if (
				this.board.confirmationPopup &&
				this.board.confirmationPopup.isOpen()
			) {
			...
			if (handler) {
				...
				if (result instanceof Promise) {
					result
						...
						.catch((error) => {
							console.error(
								chalk.red('Error in async key handler:'),
								error.message
							);
	...
	handleUnknownKey(key) {
		...
		if (process.env.DEBUG_KEYBOARD) {
			...
			const keyHex = key
				...
				.join(' ');
			console.log(
				chalk.dim(`Unknown key: "${key}" (code: ${keyCode}, hex: ${keyHex})`)
			);
		}
	}
