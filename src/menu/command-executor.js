 ...
 * @returns {Promise<Object>} - Execution result
 */
export async function executeCommandWithParams(
	command,
	...
	try {
		...
		const missingParams = parameterDefinitions
			.filter(
				(def) => def.required && (!params[def.name] || params[def.name] === '')
			)
			.map((def) => def.name);
		...
		if (result.success) {
			...
			logError(
				...
				new Error(`Exit code: ${result.exitCode}`),
				{ command, params, args }
			);
		}
		...
		console.error(
			chalk.red('Error executing command with parameters:'),
			error.message
		);
		logError('Parameter processing error', error, {
			command,
			parameterDefinitions
		});
	...
	taskId: {
		...
		validate: (input) => {
			...
			return !isNaN(num) && num > 0
				? true
				: 'Please enter a valid task ID (positive number)';
		}
	},
