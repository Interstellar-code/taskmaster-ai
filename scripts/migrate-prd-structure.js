...
function createPrdMetadataEntry(filePath, status, targetFileName) {
	...
	const prdId =
		extractedMeta.id || generatePrdIdFromFilename(path.basename(filePath));
	const title =
		extractedMeta.title || path.basename(filePath, '.md').replace(/_/g, ' ');
		...
		for (const file of files) {
			...
			try {
				...
				if (fs.existsSync(targetPath)) {
					...
					const metadata = createPrdMetadataEntry(
						sourcePath,
						status,
						newFileName
					);
	...
	for (const status of statusDirs) {
		...
		if (fs.existsSync(statusDir)) {
			...
			if (remainingFiles.length === 0) {
				...
				console.log(
					`⚠️  Directory ${status} still contains files: ${remainingFiles.join(', ')}`
				);
			}
		}
	}
