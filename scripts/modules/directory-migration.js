...
function migrateTasks(projectRoot) {
	...
	if (fs.existsSync(oldPath)) {
		...
		log('info', `Tasks migrated from ${OLD_STRUCTURE.tasks} to ${NEW_STRUCTURE.tasks}`);
	}
}
...
function migratePRD(projectRoot) {
	...
	if (fs.existsSync(oldPath)) {
		...
		copyRecursive(oldPath, newPath);
		log('info', `PRD files migrated from ${OLD_STRUCTURE.prd} to ${NEW_STRUCTURE.prd}`);
	}
}
...
function migrateReports(projectRoot) {
	...
	if (fs.existsSync(oldScriptsPath)) {
		...
		const reportFiles = files.filter(file => 
			file.includes('complexity') || file.includes('report')
		);
...
function migrateTemplates(projectRoot) {
	...
	if (fs.existsSync(oldPath)) {
		...
		copyRecursive(oldPath, newPath);
		log('info', `Templates migrated from ${OLD_STRUCTURE.templates} to ${NEW_STRUCTURE.templates}`);
	}
}
...
function migrateConfig(projectRoot) {
	...
	if (fs.existsSync(oldConfigPath)) {
		...
		log('info', `Configuration migrated from ${OLD_STRUCTURE.config} to ${NEW_STRUCTURE.config}`);
	}
}
...
export function migrateDirectoryStructure(projectRoot, preserveOld = true) {
	try {
		...
		if (!preserveOld) {
			log('info', 'Note: Old files preserved. Use --clean flag to remove them after verification.');
		}
