...
function extractPrdMetadata(filePath) {
    try {
            ...
            if (match) {
                ...
                switch (key) {
                    ...
                    case 'Completed Tasks':
                        ...
                        break;
                    case 'Completion':
                        metadata.completionPercentage = parseInt(value.replace('%', '')) || 0;
                        break;
                    case 'PRD Path':
        ...
        log('error', `Error extracting PRD metadata from ${filePath}:`, error.message);
        return {};
    }
}
...
function updatePrdFileMetadata(filePath, prdData, options = {}) {
    try {
        ...
        const hasTaskMasterMetadata = lines.some(line =>
        ...
        log('error', `Error updating PRD file metadata for ${filePath}:`, error.message);
        return false;
    }
}
...
function updatePrdFileOnParse(filePath, parseInfo = {}) {
    try {
        ...
        log('error', `Error updating PRD file on parse for ${filePath}:`, error.message);
        return false;
    }
}
...
function syncPrdFileMetadata(filePath, prdData) {
    try {
        ...
        log('error', `Error syncing PRD file metadata for ${filePath}:`, error.message);
        return false;
    }
}
