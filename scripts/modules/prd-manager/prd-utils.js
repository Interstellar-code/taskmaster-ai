...
const prdMetadataSchema = z.object({
    ...
    linkedTaskIds: z.array(z.union([z.number().int(), z.string()])).optional().default([]),
...
const prdsMetadataSchema = z.object({
    ...
    metadata: z.object({
        ...
        schema: z.object({
            version: z.string().default('1.0.0'),
            description: z.string().default('TaskMaster AI PRD Lifecycle Tracking Schema'),
...
function readPrdsMetadata(prdsPath = null) {
    try {
        ...
        if (!validationResult.success) {
            log('error', 'PRDs metadata validation failed:', validationResult.error.message);
            throw new Error(`Invalid PRDs metadata structure: ${validationResult.error.message}`);
        }
        
        return validationResult.data;
    } catch (error) {
        log('error', `Error reading PRDs metadata from ${prdsPath}:`, error.message);
        throw error;
    }
}
...
function initializePrdsMetadata(prdsPath = null) {
    const defaultStructure = {
        ...
        metadata: {
            ...
            automationRules: {
                statusTransitions: {
                    pending_to_inprogress: 'When first linked task changes to \'in-progress\'',
                    inprogress_to_done: 'When ALL linked tasks reach \'done\' status (100% completion)',
                    manual_override: 'Allow manual status changes with validation and audit trail',
                    rollback_protection: 'Prevent status regression without explicit user confirmation'
                }
            }
        }
    };
...
function getPrdStatistics(prdsPath = null) {
    try {
        ...
        const stats = {
            ...
            totalLinkedTasks: prds.reduce((sum, p) => sum + (p.linkedTasks ? p.linkedTasks.length : (p.linkedTaskIds ? p.linkedTaskIds.length : 0)), 0),
            averageTasksPerPrd: prds.length > 0 ?
                prds.reduce((sum, p) => sum + (p.linkedTasks ? p.linkedTasks.length : (p.linkedTaskIds ? p.linkedTaskIds.length : 0)), 0) / prds.length : 0
        };
