...
function analyzePrdFile(filePath) {
	try {
		...
		const titleMatch =
			content.match(/^#\s+(.+)$/m) || content.match(/^Title:\s*(.+)$/im);
		if (titleMatch) {
		...
		if (
			highPriorityKeywords.some((keyword) => contentLower.includes(keyword))
		) {
			priority = 'high';
		} else if (
			lowPriorityKeywords.some((keyword) => contentLower.includes(keyword))
		) {
			priority = 'low';
		}
 ...
 */
function findTasksReferencingPrd(
	prdFileName,
	...
	try {
		...
		return tasksData.tasks
			.filter(
				(task) => task.prdSource && task.prdSource.fileName === prdFileName
			)
			.map((task) => task.id);
	} catch (error) {
		log(
			'error',
			`Error finding tasks referencing PRD ${prdFileName}:`,
			error.message
		);
		return [];
	}
	...
	try {
		...
		if (moveToStatusDirectory) {
			...
			if (!moveResult.success) {
				log(
					'warn',
					`Failed to move PRD ${prdId} to status directory: ${moveResult.error}`
				);
			}
		}
		...
		log(
			'info',
			`Successfully migrated PRD: ${fileInfo.fileName} (${linkedTasksCount} tasks linked)`
		);
		return {
			success: true,
			data: {
