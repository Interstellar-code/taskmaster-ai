...
const STATUS_TRANSITION_RULES = {
	...
	// From done
		...
		'in-progress': {
			...
			description:
				'When any linked task is moved from done to incomplete status'
		},
		archived: {
 ...
 */
function updatePrdStatusBasedOnTasks(
	prdId,
	tasksPath = 'tasks/tasks.json',
	prdsPath = 'prd/prds.json',
	options = {}
) {
	const { force = false, dryRun = false, allowManualOverride = true } = options;
	...
	try {
		...
		log(
			'info',
			`Updated PRD ${prdId} status from ${currentStatus} to ${recommendedStatus}`
		);
		return {
			success: true,
			data: {
...
function updateAllPrdStatuses(
	...
	try {
		...
		log(
			'info',
			`Batch PRD status update completed: ${results.updated} updated, ${results.unchanged} unchanged, ${results.errors} errors`
		);
		return {
			success: true,
			data: results
...
function setupAutomatedStatusMonitoring(options = {}) {
	...
	const startMonitoring = () => {
		...
		log(
			'info',
			`Started automated PRD status monitoring (interval: ${interval}ms)`
		);
