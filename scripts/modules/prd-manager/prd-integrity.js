	...
	try {
		...
		for (const prd of prdsData.prds) {
			...
			for (const taskId of linkedTaskIds) {
				const task = tasksData.tasks.find(
					(t) => t.id === taskId || t.id === Number(taskId)
				);
		...
		for (const task of tasksData.tasks) {
			if (task.prdSource && task.prdSource.fileName) {
				const prd = prdsData.prds.find(
					(p) => p.fileName === task.prdSource.fileName
				);
...
function updatePrdTaskStatistics(prdId, tasksPath = null, prdsPath = null) {
	try {
		...
		const linkedTasks = tasksData.tasks.filter(
			(task) =>
				linkedTaskIds.includes(task.id) ||
				linkedTaskIds.includes(String(task.id))
		);
		...
		log(
			'info',
			`Updated task statistics for PRD ${prdId}: ${stats.completionPercentage}% complete`
		);
		return {
			success: true,
			data: stats
		...
		log(
			'error',
			`Error updating PRD task statistics for ${prdId}:`,
			error.message
		);
		return {
			success: false,
...
function autoFixFileIntegrityIssues(prdsPath = null) {
	...
	try {
				...
				for (const issue of fileCheck.issues) {
					try {
						if (
							...
							if (fs.existsSync(prd.filePath)) {
								...
								fixResults.details.push(
									`Updated hash and size for PRD ${prd.id} (${prd.fileName})`
								);
								prdFixed = true;
							}
							...
							if (fs.existsSync(newFilePath)) {
								...
								fixResults.details.push(
									`Updated file path for PRD ${prd.id} to correct status directory`
								);
								prdFixed = true;
							} else {
								...
								if (fs.existsSync(prd.filePath)) {
									...
									fixResults.details.push(
										`Moved PRD ${prd.id} file to correct status directory`
									);
									prdFixed = true;
								}
							}
						...
						fixResults.errors.push(
							`Failed to fix ${issue.type} for PRD ${prd.id}: ${error.message}`
						);
					}
				}
		...
		fixResults.errors.push(
			`Error during file integrity auto-fix: ${error.message}`
		);
		return fixResults;
	}
}
			...
			if (task.prdSource && task.prdSource.fileName) {
				...
				if (prd) {
					...
					if (
						...
						fixResults.details.push(
							`Added task ${task.id} to PRD ${prd.id} linkedTaskIds`
						);
					}
				}
			}
...
function performIntegrityCheck(
	...
	try {
		...
		if (options.autoFix) {
			...
			const fileIntegrityFixResults =
				autoFixFileIntegrityIssues(resolvedPrdsPath);

			// Fix missing task links
			const taskLinkFixResults = autoFixMissingTaskLinks(
				resolvedTasksPath,
				resolvedPrdsPath
			);
			...
			report.autoFixResults = {
				...
				totalFixed:
					(fileIntegrityFixResults.filesFixed || 0) +
					(taskLinkFixResults.linksFixed || 0)
			};

			// Add recommendations based on fixes
			if (
				fileIntegrityFixResults.success &&
				fileIntegrityFixResults.filesFixed > 0
			) {
				report.recommendations.push(
					`Auto-fixed ${fileIntegrityFixResults.filesFixed} file integrity issues`
				);
			}
			if (taskLinkFixResults.success && taskLinkFixResults.linksFixed > 0) {
				report.recommendations.push(
					`Auto-fixed ${taskLinkFixResults.linksFixed} missing task links`
				);
			}
		}
		...
		for (const prd of prdsData.prds) {
			...
			// Count issues
			const errors = fileCheck.issues.filter(
				(i) => i.severity === 'error'
			).length;
			const warnings = fileCheck.issues.filter(
		...
		const linkingCheck = checkTaskPrdLinkingConsistency(
			resolvedTasksPath,
			resolvedPrdsPath
		);
		report.linkingConsistency = linkingCheck;

		const linkingErrors = linkingCheck.issues.filter(
			(i) => i.severity === 'error'
		).length;
		const linkingWarnings = linkingCheck.issues.filter(
			(i) => i.severity === 'warning'
		).length;
		...
		if (report.overall.errorCount > 0) {
			report.recommendations.push(
				'Fix critical errors before proceeding with PRD operations'
			);
		}
		...
		if (!options.autoFix && report.overall.warningCount > 0) {
			const missingLinkWarnings = linkingCheck.issues.filter(
				(i) => i.type === 'missing_task_link' && i.severity === 'warning'
			).length;

			if (missingLinkWarnings > 0) {
				report.recommendations.push(
					`Run integrity check with --auto-fix to automatically fix ${missingLinkWarnings} missing task links`
				);
			}
		}
	} catch (error) {
		...
		report.recommendations.push(
			`Critical error during integrity check: ${error.message}`
		);
