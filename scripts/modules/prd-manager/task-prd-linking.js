...
function linkTaskToPrd(
	...
	try {
		...
		log(
			'error',
			`Error linking task ${taskId} to PRD ${prdId}:`,
			error.message
		);
		return {
			success: false,
...
function unlinkTaskFromPrd(
	...
	try {
		...
		log(
			'error',
			`Error unlinking task ${taskId} from PRD ${prdId}:`,
			error.message
		);
		return {
			success: false,
...
function updateTaskPrdSource(
	...
	try {
		...
		log(
			'error',
			`Error updating PRD source for task ${taskId}:`,
			error.message
		);
		return {
			success: false,
...
function syncTaskPrdLinks(
	...
	try {
			...
			if (task.prdSource && task.prdSource.fileName) {
				...
				if (prds.length === 0) {
					...
					syncResults.errors.push(
						`Multiple PRDs found for filename ${task.prdSource.fileName}`
					);
				}
			}
		}
		...
		for (const prd of prdsData.prds) {
			...
			for (const taskId of prd.linkedTaskIds) {
				...
				if (task) {
					...
					log(
						'warn',
						`Removed orphaned task link ${taskId} from PRD ${prd.id}`
					);
				}
			}
		...
		log(
			'info',
			`Sync completed: ${syncResults.linksCreated} created, ${syncResults.linksUpdated} updated, ${syncResults.linksRemoved} removed`
		);
		return {
			success: true,
			data: syncResults
...
function getTasksLinkedToPrd(
	...
	try {
		...
		return tasksData.tasks.filter(
			(task) =>
				linkedIds.includes(task.id) || linkedIds.includes(String(task.id))
		);
	} catch (error) {
