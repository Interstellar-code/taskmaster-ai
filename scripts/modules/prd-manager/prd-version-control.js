 ...
 */
function createVersionEntry(prd, changeType, changeDetails = {}, author = 'system') {
    return {
...
function trackFileChanges(prdId, options = {}) {
    ...
    try {
        ...
        // Add version entry
        const versionResult = addVersionEntry(prdId, 'file_modified', changeDetails, {
            author: author,
            versionType: 'patch',
            prdsPath: prdsPath
        });
        ...
        log('error', `Error tracking file changes for PRD ${prdId}:`, error.message);
        return {
            success: false,
            error: error.message
        };
...
function getVersionHistory(prdId, options = {}) {
    ...
    try {
        ...
        log('error', `Error getting version history for PRD ${prdId}:`, error.message);
        return {
            success: false,
            error: error.message
        };
 ...
 */
function compareVersions(prdId, version1, version2, prdsPath = 'prd/prds.json') {
    try {
        ...
        const differences = {
            ...
            tags: JSON.stringify(v1Entry.snapshot.tags) !== JSON.stringify(v2Entry.snapshot.tags),
            linkedTasks: JSON.stringify(v1Entry.snapshot.linkedTaskIds) !== JSON.stringify(v2Entry.snapshot.linkedTaskIds),
