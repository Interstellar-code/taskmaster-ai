...
import type { KanbanTask, ColumnId as ApiColumnId } from "../api/types";
...
// Convert KanbanTask to Task for compatibility with existing components
function kanbanTaskToTask(kanbanTask: KanbanTask): Task {
  return {
    id: kanbanTask.id,
    columnId: kanbanTask.columnId,
    content: kanbanTask.content,
  };
}
  ...
  useEffect(() => {
    async function loadTasks() {
      try {
        ...
        const tasksByColumns = await taskService.getTasksByColumns();

        // Convert to Task format for existing components
        const allTasks: Task[] = [];
        Object.entries(tasksByColumns).forEach(([columnId, kanbanTasks]) => {
          kanbanTasks.forEach(kanbanTask => {
            allTasks.push(kanbanTaskToTask(kanbanTask));
          });
        });
