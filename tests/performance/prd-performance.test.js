		...
		for (let i = 1; i <= count; i++) {
			tasks.push({
				...
				prdSource: i % 5 !== 0 ? {
					...
					fileHash: crypto.createHash('sha256').update(`content-${i}`).digest('hex'),
					fileSize: 1024 + (i * 100)
				} : null
			});
		}
	...
	describe('PRD Parsing Performance', () => {
		test('should parse small PRD files quickly', async () => {
			...
			const { duration } = await measureTime(async () => {
				...
				const hash = crypto.createHash('sha256').update(smallPRD, 'utf8').digest('hex');
		...
		test('should parse medium PRD files within acceptable time', async () => {
			...
			const { duration } = await measureTime(async () => {
				const hash = crypto.createHash('sha256').update(mediumPRD, 'utf8').digest('hex');
				const lines = mediumPRD.split('\n');
				const features = lines.filter(line => line.startsWith('### Feature'));
				
				// Simulate more complex parsing
				const requirements = lines.filter(line => line.includes('Requirement'));
		...
		test('should parse large PRD files within time limit', async () => {
			...
			const { duration } = await measureTime(async () => {
				const hash = crypto.createHash('sha256').update(largePRD, 'utf8').digest('hex');
		...
		test('should handle large number of tasks within time limit', async () => {
			...
			const { duration } = await measureTime(async () => {
				...
				tasks.forEach(task => {
					...
					if (task.prdSource) {
						const fileName = task.prdSource.fileName;
						analysis.byPRDSource.set(fileName, (analysis.byPRDSource.get(fileName) || 0) + 1);
					}
		...
		test('should detect changes in multiple PRD files efficiently', async () => {
			...
			uniquePRDs.forEach(fileName => {
				prdFiles.push({
					...
					originalHash: crypto.createHash('sha256').update(`original-${fileName}`).digest('hex')
				});
			});
			...
			const { duration } = await measureTime(async () => {
				...
				prdFiles.forEach(prdFile => {
					const currentHash = crypto.createHash('sha256').update(prdFile.content, 'utf8').digest('hex');
		...
		test('should handle large datasets without excessive memory usage', async () => {
			...
			const { result } = await measureTime(async () => {
				...
				const analysis = {
					...
					totalPRDSize: largePRDs.reduce((sum, prd) => sum + prd.content.length, 0),
					tasksByPRD: new Map(),
		...
		test('should handle concurrent PRD operations efficiently', async () => {
			const { duration } = await measureTime(async () => {
				...
				const operations = [
					...
					Promise.resolve(crypto.createHash('sha256').update('content1').digest('hex')),
					Promise.resolve(crypto.createHash('sha256').update('content2').digest('hex')),
					Promise.resolve(crypto.createHash('sha256').update('content3').digest('hex'))
				];
		...
		test('should maintain consistent performance across runs', async () => {
			...
			for (let i = 0; i < runs; i++) {
				const { duration } = await measureTime(async () => {
					...
					const hash = crypto.createHash('sha256').update(prdContent).digest('hex');
