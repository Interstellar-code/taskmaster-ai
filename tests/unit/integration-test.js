...
describe('v0.16.0 Integration Tests', function() {
	...
	describe('Phase 2: AI Provider Integration', function() {
		it('should have BaseAIProvider class', async function() {
			const { BaseAIProvider } = await import('../../src/ai-providers/base-provider.js');
			expect(BaseAIProvider).to.be.a('function');
			
			// Test that it cannot be instantiated directly
			expect(() => new BaseAIProvider()).to.throw('BaseAIProvider cannot be instantiated directly');
		});
		...
		it('should have updated supported models', function() {
			const supportedModels = JSON.parse(fs.readFileSync(
	...
	describe('Phase 3: Directory Structure Migration', function() {
		it('should have new .taskmaster directory structure', function() {
			...
			expect(fs.existsSync(path.join(projectRoot, '.taskmaster/tasks'))).to.be.true;
			expect(fs.existsSync(path.join(projectRoot, '.taskmaster/prd'))).to.be.true;
			expect(fs.existsSync(path.join(projectRoot, '.taskmaster/reports'))).to.be.true;
			expect(fs.existsSync(path.join(projectRoot, '.taskmaster/templates'))).to.be.true;
		});

		it('should have migrated configuration file', function() {
			expect(fs.existsSync(path.join(projectRoot, '.taskmaster/config.json'))).to.be.true;
		});
		...
		it('should have migrated tasks', function() {
			...
			if (fs.existsSync(tasksDir)) {
				...
				const taskFiles = files.filter(f => f.startsWith('task_') && f.endsWith('.txt'));
	...
	describe('Configuration Management', function() {
		it('should load config from new location', async function() {
			const { loadConfig } = await import('../../scripts/modules/config-manager.js');
		...
		it('should support both old and new config paths', async function() {
			const { getStructurePath } = await import('../../scripts/modules/directory-migration.js');
	...
	describe('Unique Features Preservation', function() {
		it('should preserve Kanban board functionality', function() {
			...
			expect(fs.existsSync(path.join(projectRoot, 'src/menu/kanban'))).to.be.true;
		});
		...
		it('should preserve interactive menu system', function() {
			expect(fs.existsSync(path.join(projectRoot, 'src/menu/index.js'))).to.be.true;
		});
	});
...
describe('AI Provider Functionality Tests', function() {
	...
	describe('Provider Class Methods', function() {
		...
		it('should handle validation correctly', async function() {
			const { AnthropicAIProvider } = await import('../../src/ai-providers/index.js');
			...
			expect(() => provider.validateParams({ apiKey: 'test' })).to.throw('Model ID is required');
		});
	});
});
