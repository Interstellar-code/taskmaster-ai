...
/**
 * GET /api/v1/tasks/:id/complexity
 * Analyze task complexity
 */
router.get('/tasks/:id/complexity', requireTaskMasterCore, async (req, res) => {
  const { id } = req.params;
  const tasksJsonPath = req.app.locals.tasksJsonPath;
  const projectRoot = req.app.locals.projectRoot;
...
router.get('/reports/complexity', requireTaskMasterCore, async (req, res) => {
  ...
  // Construct the default report path based on project structure
  const reportPath = path.join(projectRoot, '.taskmaster', 'reports', 'complexity-report.json');

  await handleDirectFunction(
    complexityReportDirect,
    { tasksJsonPath, projectRoot, reportPath },
    res,
    'complexityReportDirect'
  );
});
